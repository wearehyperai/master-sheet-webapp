        ...
        {nodes.map((node, index) => (
          ...
          >
            ...
            {isNodeSelectable(index) ? (
              <>
                {index === currentNode ? (
                  ...
                  isScraperMode && index === 0 ? (
                    <div className="mt-2 text-sm font-medium text-emerald-700 p-2 bg-emerald-50 rounded-md">
                      Selected: {getOptionById(index, initialScraperId || '')?.name || "Scraper"}
                    </div>
                  ) : (
                    // Otherwise, show the dropdown to select an option
                    <select 
                      className="mt-2 block w-full px-3 py-2 text-sm border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-emerald-500"
              ...
              <div className="mt-2 text-xs italic text-gray-500 p-2 bg-gray-50 rounded-md">
