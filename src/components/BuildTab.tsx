...
const BuildTab: React.FC<BuildTabProps> = ({
  ...
  const isNodeSelectable = (nodeIndex: number): boolean => {
  ...
  // Get option by ID - safely handle optional options property
  const getOptionById = (nodeIndex: number, optionId: string): NodeOption | undefined => {
    // Check if the node at nodeIndex exists and has options property
                  ...
                  <>
                    {index === currentNode ? (
                      ...
                      isScraperMode && index === 0 ? (
                        ...
                        // Otherwise, show the dropdown to select an option - check if options exist
                        node.options ? (
                          <select 
                          ...
                          >
                            ...
                            {node.options.map((option: NodeOption) => (
